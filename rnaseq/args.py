...
def get_args():
    ...
    PARSER.add_argument('-M', '--max-len', dest='maxLength', help='provide maximum length required'
                                                                  ' to filter the reads', metavar='\b', required=False)
    PARSER.add_argument('-q', '--qualified_quality_phred', dest='qualifiedPhred',
                        help='the quality value that a base is qualified. Default 15 means phred quality >=Q15 is '
                             'qualified.',
                        metavar='\b', required=False)
    ...
    PARSER.add_argument('-s', '--strandness', dest='strandness', help='Perform strand-specific read counting.'
                                                                      '0 (unstranded), 1 (stranded) and 2 (reversely '
                                                                      'stranded). Default is 0',
                        required=False, metavar='\b', type=int)
